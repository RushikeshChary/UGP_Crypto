...
awaitable<void> run_p0(io_context& io, int m) {
    ...
    // Write to p0_data.bin for verification
    std::ofstream out("p0_data.bin", std::ios::binary);
    uint64_t m_val = m;
    out.write((char*)&m_val, sizeof(uint64_t));
    out.write((char*)messages.data(), sizeof(BlockPair) * m);
    out.close();
...
awaitable<void> run_p1(io_context& io, int m) {
    ...
    // Write to p1_data.bin for verification
    std::ofstream out("p1_data.bin", std::ios::binary);
    uint64_t m_val = m;
    out.write((char*)&m_val, sizeof(uint64_t));
    out.write((char*)choices.data(), m);
    out.write((char*)results.data(), sizeof(__m128i) * m);
    out.close();
